...
| 目录 | 说明 |
|------|------|
| **`backend/`** | FastAPI 应用：`app/` 包、`requirements.txt`、`pyproject.toml`、`Dockerfile` |
| **`frontend/`** | Vue 3 + Vite 前端与 `Dockerfile.frontend` |
| 根目录 | `docker-compose.yml`、`.env`、数据与部署相关配置 |

内置站点元数据（`sites.json`）位于 **`backend/app/config/sites.json`**，随镜像内 `app` 包一起发布。

## 🚀 快速开始

本项目推荐使用 Docker Compose 部署。**请克隆完整仓库后在项目根目录执行 Compose**，构建上下文为 `./backend` 与 `./frontend`。不要只下载单独的 `docker-compose.yml`，否则无法构建镜像。
...
- 若在 Compose 里把宿主机 **空的或不完整的 `./backend/app`** 挂载到容器内 **`/app/app`**，会覆盖镜像内的代码导致导入失败。默认提供的 `docker-compose.yml` **不应**挂载后端源码目录；需要改代码热重载时请使用下文「后端开发（Docker）」中的 **`docker-compose.dev.yml`**。
...
后端基于 FastAPI + Python 3.11，工作在仓库的 **`backend/`** 目录下。
...
使用开发覆盖配置，挂载 **`backend/app/`** 并以 `--reload` 启动：
